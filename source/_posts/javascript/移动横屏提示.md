---
title:         移动端横屏提示 # 标题
description:   移动端开发时横屏提示 # 副标题
date:          2017-04-03 # 建立日期
comments:      true  # 开启评论功能
tags: # 标签分类
    - JavaScript
---


做移动端开发的时候需要对横屏做处理，否则用户体验很不好

css和js均有检测横屏的机制，但是想要把一个竖屏的网页在横屏上显示出来，各种方法都不太合适

除非制作的是横屏的游戏，否则一律在用户横屏的时候提示用户需竖屏访问

所以增加一段js代码，提示横屏

效果如下：
<img src="../images/javascript/landscape.jpg" width="400px">

代码如下：

```js
(function() {
  var landscapeTip;
  function createLandscapeTip() {
    if (!landscapeTip) {
      landscapeTip = document.createElement('div');
      landscapeTip.style.background = 'black';
      landscapeTip.style.position = 'fixed';
      landscapeTip.style.width = '100%';
      landscapeTip.style.height = '100%';
      landscapeTip.style.left = 0;
      landscapeTip.style.top = 0;
      landscapeTip.style.textAlign = 'center';
      landscapeTip.style.zIndex = 99999999999;
      landscapeTip.style.overflow = 'hidden';
      landscapeTip.innerHTML = '<div  style="position: absolute; top: 50%;   left: 50%; -webkit-transform:translate(-50%,-50%);transform:translate(-50%,-50%)"><img style="height:150px" src="data:image/png;base64,"/><br><div style="height: 1.6rem; color: white; font-size: 18px; text-align: center;">竖屏浏览,体验更佳</div></div>';
      document.body.appendChild(landscapeTip);

    }
  }

  function showLandscapeTip() {
    if (landscapeTip) {
      landscapeTip.style.display = 'block';
    }
  }

  function hideLandscpeTip() {
    if (landscapeTip) {
      landscapeTip.style.display = 'none';
    }
  }

  function updateOrientation() {
    var orientation = window.orientation;
    switch (orientation) {
      case 90:
      case -90:
        orientation = 'landscape';
        showLandscapeTip();
        break;
      default:
        orientation = 'portrait';
        hideLandscpeTip();
        break;
    }
  }

  function initOrientationChange() {
    createLandscapeTip();
    window.addEventListener('orientationchange', updateOrientation, false);
    landscapeTip.addEventListener('touchstart', function(e) {
      e.preventDefault();
    }, false);
    updateOrientation();
    // showLandscapeTip(); // debug
  }
  initOrientationChange();

})();
```


一般情况下使用上面的脚本就可以了，当时当网页以横屏方式进入时，landscape.js脚本为加载完前，这个时候网页可能会出问题，我们可以通过宽高判断来手动刷新
```
(function () {
  var landscapeTip
  var checkTimer
  var isInitPortrait = true

  function createLandscapeTip () {
    if (!landscapeTip) {
      landscapeTip = document.createElement('div')
      landscapeTip.style.background = 'black'
      landscapeTip.style.position = 'fixed'
      landscapeTip.style.width = '100%'
      landscapeTip.style.height = '100%'
      landscapeTip.style.left = 0
      landscapeTip.style.top = 0
      landscapeTip.style.textAlign = 'center'
      landscapeTip.style.zIndex = 99999999999
      landscapeTip.style.overflow = 'hidden'
      landscapeTip.innerHTML = '<div  style="position: absolute; top: 50%;   left: 50%; -webkit-transform:translate(-50%,-50%);transform:translate(-50%,-50%)"><img style="height:150px" src="data:image/png;base64,"/><br><div style="height: 1.6rem; color: white; font-size: 18px; text-align: center;">竖屏浏览,体验更佳</div></div>'
      document.body.appendChild(landscapeTip)
    }
  }

  function showLandscapeTip () {
    if (landscapeTip) {
      landscapeTip.style.display = 'block'
    }
  }

  function hideLandscapeTip () {
    if (landscapeTip) {
      landscapeTip.style.display = 'none'
    }
  }

  function updateOrientation () {
    var orientation = window.orientation
    switch (orientation) {
      case 90:
      case -90:
        orientation = 'landscape'
        showLandscapeTip()
        break
      default:
        orientation = 'portrait'
        hideLandscapeTip()
        break
    }
  }

  function initCheck () { // initCheck的目的在于保证在updateOrientation()不调用的情况下保证横屏进入后切换成竖屏时能刷新页面重置字号
    if (window.document.documentElement.clientWidth > window.document.documentElement.clientHeight) {
      isInitPortrait = false
      showLandscapeTip()
    }
    // 设置定时器检测横竖屏切换
    checkTimer = setInterval(function () {
      if (window.document.documentElement.clientWidth < window.document.documentElement.clientHeight) { // 竖屏状态
        if (!isInitPortrait) {
          // 给重定向增加参数nocache，防止缓存，先判断有误，再增加
          if (window.location.href.indexOf('nocache') > -1) {
            window.location.replace(window.location.href.substring(0, window.location.href('nocache') - 1) + '&nocache=' + Math.floor(Math.random() * 1000))
          } else {
            window.location.replace(window.location.href + '&nocache=' + Math.floor(Math.random() * 1000))
          }
        }
      }
    }, 500)
    setTimeout(function () { clearInterval(checkTimer) }, 3000) // 3s清除定时器
  }

  function initOrientationChange () {
    createLandscapeTip()
    setTimeout(function () { initCheck() }, 0)
    window.addEventListener('orientationchange', updateOrientation, false)
    landscapeTip.addEventListener('touchstart', function (e) {
      e.preventDefault()
    }, false)
    updateOrientation()
    // showLandscapeTip() // debug
  }
  initOrientationChange()
})()

```

