...
<table><tr><td>模式分类</td><td>名称</td></tr><tr><td rowspan="3">创建型</td><td>[工厂模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E5%B7%A5%E5%8E%82%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[单例模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[原型模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E5%8E%9F%E5%9E%8B%E6%A8%A1%E5%BC%8F.md)</td><tr><td rowspan="2">结构型</td><td>[适配器模式](https://github.com/sunzhaoye/blog/blob/masterdesignPattern/%E9%80%82%E9%85%8D%E5%99%A8%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[代理模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E4%BB%A3%E7%90%86%E6%A8%A1%E5%BC%8F.md)</td><tr><td rowspan="5">行为型</td><td>[策略模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E7%AD%96%E7%95%A5%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[迭代器模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E8%BF%AD%E4%BB%A3%E5%99%A8%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[观察者模式(发布-订阅模式)](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E8%A7%82%E5%AF%9F%E8%80%85%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[命令模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E5%91%BD%E4%BB%A4%E6%A8%A1%E5%BC%8F.md)</td></tr><tr><td>[状态模式](https://github.com/sunzhaoye/blog/blob/master/designPattern/%E7%8A%B6%E6%80%81%E6%A8%A1%E5%BC%8F.md)</td></tr></table>


### 工厂模式
```js
// 工厂模式：创建对象时不会对客户端暴露创建逻辑，通过使用一个通过的接口来指向新创建的对象，用工厂方法代替new
...
1. 原生js实现
  ...
  function jsonp (opt) {
    ...
    // script一旦加载表名请求已经发送，可以开始移除相应的方法
...
### CommonJS
`CommonJS` 规范是通过 `module.exports` 来定义的，在前端浏览器中，并不支持 `module.exports`，`Nodejs` 端是使用 `CommonJS` 规范的，前端浏览器一般使用 `AMD`、`CMD`、`ES6`等定义模块化开发的。
```js
var x = 5;
...
---
## Vue双向绑定原理
> 参考：[segmentfault 剖析Vue原理&实现双向绑定MVVM](https://segmentfault.com/a/1190000006599500)、[vue的双向绑定原理及实现](https://www.cnblogs.com/libin-1/p/6893712.html)
...
另外，vuex 提供了 `mapState`、`mapGetters`、`mapActions`、`mapMutations` 这些辅助函数
...
`call` 、`apply` 、`bind` 是 `Function` 对象自带的三个方法，都是为了改变函数体内部 `this` 的指向；
...
---
## `[]==false` 为什么为true，`!![]==false` 为什么为false

> [segmentfault](https://segmentfault.com/a/1190000016555947?utm_source=tag-newest)
js隐式转换：
1. 数字 == 字符串    =>  数字 == 数字
2. 数字 == 布尔值    =>  数字 == 数字
3. 字符串 == 布尔值  =>  数字 == 数字
...
***注意***：当且仅当与原始值比较时，对象会被转换为原始值。当两个操作数均为对象时，它们作为对象进行比较，仅当它们引用相同对象时返回trues
字符串对象的类型是对象，不是字符串！
```js
...
```
效果如下：
<img src="../images/tools/string_newstring.png" title="String和new String()的区别" />
...
解析：
  ...
  <img src="../images/javascript/js_zhizhen1.jpg" title="" />

  2. a中没有x属性，所以 `a.x` 为undefined，即a指向 `{n: 1, x: undefined}`。由于a、b共用存储地址，所以b也指向 `{n: 1, x: undefined}`。
  <img src="../images/javascript/js_zhizhen2.jpg" title="" />

  3. 开始赋值运算，自右向左先执行 `a = {n: 2}`。在堆中创建对象B，即 `{n: 2}`，a新指向了对象B，b暂未改变。
  <img src="../images/javascript/js_zhizhen3.jpg" title="" />

  4. 再执行 `a.x = a`，<span style="color:red">**由于一开始js已经先计算了 `a.x`，便已经解析了这个 `a.x` 是对象A的x，所以在同一条公式的情况下再回来给 `a.x` 赋值，也不会说重新解析这个 `a.x` 为对象B的x。**</span>
  相当于把对象A的x属性指向了对象B。
  导致 `b = {n: 1, x: {n: 2}}`，a未改变 `a = {n: 2}`
  <img src="../images/javascript/js_zhizhen4.jpg" title="" />
